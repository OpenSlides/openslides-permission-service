...
func Assignment(dp dataprovider.DataProvider) perm.ConnecterFunc {
	...
	return func(s perm.HandlerStore) {
		s.RegisterWriteHandler("assignment_candidate.create", perm.WriteCheckerFunc(a.createCandidate))
	}
}
...
func (a *assignment) createCandidate(ctx context.Context, userID int, payload map[string]json.RawMessage) (map[string]interface{}, error) {
	...
	if userID == cid {
		requiredPerm = "can_nominate_self"
	}
